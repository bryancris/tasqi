...
export function DatePickerInput({
  ...
  const [tempDate, setTempDate] = useState<Date>(date ? parse(date, 'yyyy-MM-dd', today) : today);
  ...
  return <div className="space-y-2">
      ...
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent className="w-[400px] p-0 bg-background border shadow-lg relative z-[100]" align="start" onInteractOutside={e => {
        e.preventDefault();
      }} sideOffset={5} style={{
        pointerEvents: 'auto'
      }}>
          ...
          <div className="p-6 relative z-[101] bg-background rounded-md">
            <div className="grid grid-cols-3 gap-8 h-[300px] mb-4">
              <MonthColumn currentDate={currentDate} tempDate={tempDate} onMonthSelect={handleMonthSelect} />
              <DayColumn tempDate={tempDate} onDaySelect={handleDaySelect} />
              <YearColumn currentDate={currentDate} tempDate={tempDate} onYearSelect={handleYearSelect} />
            </div>

            <DatePickerControls onSet={handleSetDate} onCancel={() => {
