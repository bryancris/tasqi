...
import { getDirectUserId } from "@/utils/direct-auth";
...
export function useTasks() {
  const fetchTasks = async () => {
    ...
    try {
      // Get current user ID directly instead of using auth.getUser
      const userId = await getDirectUserId();
      
      if (!userId) {
        console.warn('No user ID available - returning empty task list');
        return [];
      }
  ...
  const { data: tasks = [], refetch, isLoading } = useQuery({
    queryKey: ['tasks'],
    queryFn: fetchTasks,
    // Improved settings for faster updates and retry logic
    staleTime: 0, // Consider data always stale to ensure it's refreshed
    gcTime: 5 * 60 * 1000, // Keep unused data for 5 minutes
    refetchOnWindowFocus: true,
    refetchOnMount: true,
    refetchOnReconnect: true,
    retry: 3, // Increased retries for transient auth issues
